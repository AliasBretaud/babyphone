...
httpServer.listen(3000, () =>
  console.log("HTTP redirector on :3000 -> https://<host>:3443")
);
...
const keyPath =
  ...
  path.join(
    process.env.CERT_DIR || path.join(__dirname, "certs"),
    "server.key"
  );
const crtPath =
  process.env.CERT_CRT ||
  path.join(
    process.env.CERT_DIR || path.join(__dirname, "certs"),
    "server.crt"
  );
...
if (!fs.existsSync(keyPath) || !fs.existsSync(crtPath)) {
  ...
  console.error(
    "If using docker compose, a certgen init service should create them automatically."
  );
  process.exit(1);
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (data) => {
    ...
    if (
      ...
      if (msg.targetId) {
        [...r.broadcasters, ...r.viewers].forEach(
          (p) => p.meta?.id === msg.targetId && targets.add(p)
        );
      } else {
        (ws.meta.role === "broadcaster" ? r.viewers : r.broadcasters).forEach(
          (p) => targets.add(p)
        );
      }
...
httpsServer.listen(3443, () =>
  console.log("HTTPS BabyPhone on https://0.0.0.0:3443")
);
